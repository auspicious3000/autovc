...
class Encoder(nn.Module):
        ...
        for i in range(3):
            conv_layer = nn.Sequential(
                ConvNorm(80 + dim_emb if i == 0 else 512,
                         ...
                         padding=2,
                         dilation=1,
                         w_init_gain='relu'), nn.BatchNorm1d(512))
    ...
    def forward(self, x, c_org):
        ...
        for i in range(0, outputs.size(1), self.freq):
            codes.append(
                torch.cat(
                    (out_forward[:, i + self.freq - 1, :], out_backward[:,
                                                                        i, :]),
                    dim=-1))
...
class Decoder(nn.Module):
    ...
    def __init__(self, dim_neck, dim_emb, dim_pre):
        ...
        self.lstm1 = nn.LSTM(dim_neck * 2 + dim_emb,
                             dim_pre,
                             1,
                             batch_first=True)
        ...
        for i in range(3):
            conv_layer = nn.Sequential(
                ConvNorm(dim_pre,
                         ...
                         padding=2,
                         dilation=1,
                         w_init_gain='relu'), nn.BatchNorm1d(dim_pre))
            convolutions.append(conv_layer)
        self.convolutions = nn.ModuleList(convolutions)
        ...
        self.convolutions.append(
            nn.Sequential(
                ConvNorm(512,
                         ...
                         padding=2,
                         dilation=1,
                         w_init_gain='linear'), nn.BatchNorm1d(80)))
